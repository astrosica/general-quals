...
\begin{figure}[t]
    ...
    {\includegraphics[width=12cm]{figures/SolarDegeneracyEvolution.png}}
\end{figure}
...
{\noindent}\textbf{Rapid rotation and conservation of angular momentum}: Several properties of neutron stars were anticipated before they were observed. For example, neutron stars must rotate very rapidly. If the iron core of the pre-SN supergiant star were rotating even slowly, the decrease in radius would be so great that the conservation of angular momentum would guarantee the formation of a rapidly rotating neutron star. Assuming that the progenitor core is characteristic of a white dwarf composed entirely of iron, the ratio of the core radius to that of the resulting neutron star is given by

\begin{align*}
    \frac{}{} \approx \frac{}{} \left(\frac{}{}\right)^{5/3} = 512,
\end{align*}

{\noindent}where $Z/A=26/56$ has been used for iron. After applying conservation of angular momentum to the collapsing core (which is assumed to lose no mass, so $M_{\rm core}=M_{\rm wd}=M_{\rm ns}$), we obtain for the rotation period $P$

\begin{align*}
    P_{\rm ns} \approx 3.8\times10^{-6}P_{\rm core}.
\end{align*}

{\noindent}The question of how fast the progenitor core may be rotating is difficult to answer. As a star evolves, its contracting core is not completely isolated from the surrounding envelope, so one cannot generally use this simple approach of conservation of angular momentum. Taking the white dwarf $40$ Eridani B as an example, the observed rotation period of the core is $P_{\rm core}=1350\,{\rm s}$. Using this to estimate the resulting neutron star rotation period, $P_{ns}=5\times10^{-3}\,{\rm s}$. Thus neutron stars will be rotating very rapidly when they are formed, with rotation periods on the order of a few milliseconds.
